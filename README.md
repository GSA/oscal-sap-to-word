...
The application automates the manual mappings from OSCAL SAP to MS Word. The mappings include elements on pages in the document template and maps most of the xml elements (source file) to word without any manual intervention.   Only XML elements that match the content mappings in the included FedRAMP templates will render.
